...
bool ModuleRegistry::add(INotifier* notifier) {
    if (!notifier)
        return false;
...
QStringList ModuleRegistry::targetNames() const {
    return m_targets.keys();
}

void ModuleRegistry::dispatch(const Notification& n, QStringList* queued, QStringList* unknown) {
    ++m_received;
...
QJsonArray ModuleRegistry::targetsJson() const {
    QJsonArray arr;

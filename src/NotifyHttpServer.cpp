...
NotifyHttpServer::NotifyHttpServer(NotifyConfig config, NotifierRegistry* registry, QObject* parent)
    : QObject(parent),
      m_config(std::move(config)),
      m_registry(registry),
      m_server(new QTcpServer(this)) {
    connect(m_server, &QTcpServer::newConnection, this, &NotifyHttpServer::onNewConnection);
}

NotifyHttpServer::~NotifyHttpServer() = default;

bool NotifyHttpServer::start() {
...
void NotifyHttpServer::stop()            { m_server->close(); }
bool NotifyHttpServer::isListening() const { return m_server->isListening(); }
quint16 NotifyHttpServer::port() const   { return m_server->isListening() ? m_server->serverPort() : 0; }

void NotifyHttpServer::onNewConnection() {
...
void NotifyHttpServer::onSocketReadyRead(QTcpSocket* sock) {
...
void NotifyHttpServer::handleRequest(QTcpSocket* sock, const QByteArray& method,
...
QByteArray NotifyHttpServer::handleNotify(const QByteArray& body, int& code, QByteArray& reason) const {
    QJsonParseError pe{};
...
QByteArray NotifyHttpServer::buildStatusJson() const {
    QJsonObject o;
...
void NotifyHttpServer::reply(QTcpSocket* sock, int code, const QByteArray& reason,

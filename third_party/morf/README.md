...
Copie autonome de la bibliothèque partagée de morfredus utilisée par morfNotify :

- `beacon/` — **morfBeacon** : supervision LAN (heartbeat UDP « je suis actif » +
  endpoint HTTP `/status`). morfNotify s'en sert pour s'annoncer sur le réseau.

> morfNotify n'embarque **que** morfBeacon (contrairement à ComponentHub, qui

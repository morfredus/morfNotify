...
// -----------------------------------------------------------------------------
// TargetDef : declaration d'UNE destination a activer (lue depuis la config).
//
//   name   : identifiant utilise par les producteurs dans "targets" ("pixel")
//   type   : identifiant de fabrique ("log", "webhook", ...) -> NotifierFactory
...
// -----------------------------------------------------------------------------
// NotifyConfig : configuration complete du service. Charge depuis un fichier
...
struct NotifyConfig {
    ...
    QVector<TargetDef> targets;

    static NotifyConfig fromJson(const QJsonObject& root) {
        NotifyConfig c;
